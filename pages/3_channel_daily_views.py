...
with st.spinner("查詢與彙整中, 請稍等..."):
    ...
    if end_date < cutoff_date:
        segments.append((start_date, end_date, 23))
    elif start_date >= cutoff_date:
        segments.append((start_date, end_date, 5))
    else:
        # 跨越 cutoff：先處理 start .. cutoff-1（23），再處理 cutoff .. end（5）
        segments.append((start_date, cutoff_date - timedelta(days=1), 23))
        segments.append((cutoff_date, end_date, 5))

    dfs = []
    for seg_start, seg_end, hr in segments:
        query = f"""
        SELECT 
            a.*,
        ...
        WHERE 
            a.date BETWEEN '{seg_start.strftime('%Y-%m-%d')}' AND '{seg_end.strftime('%Y-%m-%d')}'
            AND a.channel_name  = '{P3_channel_select}'
            AND HOUR(a.time) = {hr}
            AND HOUR(b.time) = {hr}
        ORDER BY views DESC;
        """
        df_seg = SQL.query_data('yt_channel_statistics', query)
        query_save_to_db(query, start_time, len(df_seg))
        if not df_seg.empty:
            dfs.append(df_seg)
...
channel_name = latest_row['channel_name']
channel_thumbnail = latest_row['channel_thumbnails']  # 這裡要是 URL（不是 dict）
...
P3_elapsed_time = end_time - start_time
st.session_state['P3_elapsed_time'] = round(P3_elapsed_time, 2)
st.text(f"\n運行時間: {st.session_state['P3_elapsed_time']} 秒")