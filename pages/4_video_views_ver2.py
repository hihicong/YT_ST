...
def video_views(start_date_df, end_date_df):
    ...
    # merged = merged[
    # (merged["video_view_count_start"] != 0) & 
    # (merged["published_date"] < start_date)
    # ]
...
with col1:
    ...
    P4_category_select = st.selectbox("請選擇頻道類別：", category_options, index=default_index, key="P4_category_select_input")
...
with col2:
    ...
    # 類別選擇
    P4_channel_select = st.selectbox("請選擇頻道：", channel_option, index=default_index, key="P4_channel_select_input")
...
with col3:
    ...
    P4_kind_select = st.selectbox("請選擇影片類型：", kind_options, index=default_index, key="P4_kind_select_input")
